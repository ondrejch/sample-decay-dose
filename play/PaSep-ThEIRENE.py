...
def read_and_decay():
    """ Read all ThEIRENE.F71 files, get U233 production rate, decay Pa by Origen, gen Ac225 production rate """
    for my_path, my_run in runs.items():
        ...
        runs[my_path]['ac225 Bq in 5y'] = ac225_5y_Bqs
...
def analyze_results():
    ...
    with open(results_json, 'r') as f:
        runs: dict = json5.load(f)
    df = pd.DataFrame.from_dict(runs, orient='index')
    writer = pd.ExcelWriter('theirene_u233.xlsx')
    df.to_excel(writer, 'u233 decay')
    writer.close()
